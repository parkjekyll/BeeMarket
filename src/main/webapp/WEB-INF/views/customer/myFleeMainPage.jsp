...
function sortList(e){
	...
	xmlhttp.onreadystatechange = function(){
		if(xmlhttp.readyState==4 && xmlhttp.status == 200){
		    var obj = JSON.parse(xmlhttp.responseText);
				var OrderProductBox = document.getElementById("orderFleeList");
		    ...
		    for(data of obj.sortOrderData){
		    	...
		    	row6Box.setAttribute("onclick",'orderDetail('+data.fleeMarketCartVO.fleemarketcart_no+')');
		    	...
		    	small2Box.innerText= data.productDetailVO.productdetail_price +'원';
	...
	xmlhttp.send("order_status_no="+orderStatusNo+"&customer_no="+customer_no);
		...
		function orderDetail(fleemarketcart_no){
			var fleeMarketCartNO = fleemarketcart_no;
			var customer_no = ${sessionCustomer.customer_no};
			//AJAX 호출.....
			var xmlhttp = new XMLHttpRequest();
			...
			xmlhttp.onreadystatechange = function(){
				if(xmlhttp.readyState==4 && xmlhttp.status == 200){
				    ...
				    var fleemarketdetail_option = document.getElementById("fleemarketdetail_option");
				    var fleemarket_price = document.getElementById("fleemarket_price");
				    var orderflee_count = document.getElementById("orderflee_count");
				    var orderflee_price = document.getElementById("orderflee_price");
				    ...
				    var data=obj.FleeOrderDetailData;
				    var addressData=obj.addressData;
				    	...
				    	/* address_number.innerText=addressData.addressListVO.address_number;
				    	address_location.innerText=addressData.addressListVO.address_location; */
				    	customer_phone.innerText=data.customerVO.customer_phone;
				    	
				    	fleemarketdetail_option.innerText=data.fleeMarketDetailVO.fleemarketdetail_option;
				    	fleemarket_price.innerText=data.fleeMarketVO.fleemarket_price+'원';
				    	orderflee_count.innerText=data.fleeMarketCartVO.fleemarketdetail_count+'개';
				    	orderflee_price.innerText=(data.fleeMarketVO.fleemarket_price)*(data.fleeMarketCartVO.fleemarketdetail_count)+'원';
			...
			xmlhttp.open("post","../customer/getFleeOrderDetail.do" , true);
			xmlhttp.setRequestHeader("Content-type","application/x-www-form-urlencoded");
			xmlhttp.send("fleemarketcart_no="+fleeMarketCartNO+"&customer_no="+customer_no);
		}
						...
						<div class="col">
							...
							<div class="row" style="height: 166px;">
								<div class="card col" style="margin-bottom:10px; margin-top: 10px; border: 0px;">
									<div class="row g-0">
										<div class="col-2 md-4 img-thumbnail" onclick="location.href='${pageContext.request.contextPath }/fleemarket/readFleeMarketContentPage.do?fleemarket_no=${data.fleeMarketVO.fleemarket_no }'" 
											style="background-image: url('/upload_files/${ data.fleeMarketImageVO.fleemarketimage_location }'); background-repeat: no-repeat; background-size: cover; background-position: center; border:0px; height:145px;">
										</div>
										<div class="col md-6">
											<div class="row" style="color: #5b5b5b; font-size: 18px; font-weight: bold; text-align: left; padding-left: 30px;" onclick="location.href='${pageContext.request.contextPath }/product/productDetailPage.do?product_no=${data.productVO.product_no }'">
												${data.fleeMarketVO.fleemarket_name }	
											</div>
											<hr class="one" style="margin-bottom: 10px; margin-top: 10px; color: #fcc92f;">
											<div class="row">
											<div class="col">
												<div class="row" onclick="orderDetail(${data.fleeMarketCartVO.fleemarketcart_no })" style="padding-left: 21px;">
													<p style="padding-bottom: 7px; margin-bottom: 0px;">주문번호 <small class="text-muted">${data.fleeMarketCartVO.fleemarketcart_no }</small></p>
													<p style="padding-bottom: 7px; margin-bottom: 0px;">수 량 <small class="text-muted">${data.fleeMarketCartVO.fleemarketdetail_count }</small></p>
													<p style="padding-bottom: 7px; margin-bottom: 0px;">결제금액 <small class="text-muted">${(data.fleeMarketVO.fleemarket_price) * (data.fleeMarketCartVO.fleemarketdetail_count) }원</small></p>
												</div>
											</div>
						...
						<h4 style="color: #5b5b5b; font-size: 25px; font-weight: bold; text-align: left; padding-top: 10px; padding-left: 20px;">주문상품</h4>
							<table class="table">
								<tbody>
									...
									<tr>
										<td id="fleemarketdetail_option">옵션</td>
										<td id="fleemarket_price">원</td>
										<td id="orderflee_count">개</td>
										<td id="orderflee_price">총 원</td>
									</tr>
								</tbody>
							</table>
						</div>
