...
function refreshForm(){
   ...
   xmlhttp.onreadystatechange = function(){
      if(xmlhttp.readyState== 4 && xmlhttp.status == 200){
         ...
         if(orderList.orderVO.order_status_no==6 || orderList.orderVO.order_status_no ==7|| orderList.orderVO.order_status_no ==8 ||orderList.orderVO.order_status_no ==9 ||orderList.orderVO.order_status_no ==10 || orderList.orderVO.order_status_no ==11 || orderList.orderVO.order_status_no ==12 || orderList.orderVO.order_status_no ==13){
         ...
         setTable.appendChild(tr2);
                ...
                tr2.appendChild(std4);
                 ...
                 if(orderList.orderVO.order_status_no ==7|| orderList.orderVO.order_status_no ==8 ||orderList.orderVO.order_status_no ==9 || orderList.orderVO.order_status_no ==10 || orderList.orderVO.order_status_no ==11 || orderList.orderVO.order_status_no ==12 ||orderList.orderVO.order_status_no ==13){
                ...
                std7.setAttribute("style","color:red; font-weight: bold;")
                ...
                var std8 = document.createElement("td");
                 std8.setAttribute("id","description");
                 var std8_button = document.createElement("button");
                 std8_button.setAttribute("type","button");
                 //std8_button.setAttribute("class","btn btn-danger btn-sm");
                 std8_button.setAttribute("style","height:30px;");
                 std8_button.innerText="보기";
                if(orderList.orderVO.order_status_no==6){
                   std8_button.setAttribute("onclick","location.href='cancelTemp.do?order_no="+orderList.orderVO.order_no+"'");
                }else if(orderList.orderVO.order_status_no==7 || orderList.orderVO.order_status_no==9 || orderList.orderVO.order_status_no==11){
                   std8_button.setAttribute("onclick","location.href='changeTemp.do?order_no="+orderList.orderVO.order_no+"'");
                }else{
                   std8_button.setAttribute("onclick","location.href='refundTemp.do?order_no="+orderList.orderVO.order_no+"'");
                }
                std8.appendChild(std8_button);
                tr2.appendChild(std8);
                tableBox.appendChild(setTable); 
   ...
   <div id="main">
      ...
      <div id="wrapper">
         ...
         <select id="arrayPage" onchange="location.href=this.value" class="form-control">
         	...
         	<option value="./refundList.do?seller_no=${sessionSeller.seller_no }" selected>취소내역</option>
         </select>
         <hr>
         <div id="tableBox"></div>
      </div>
